...
- **Win11 适配（第二轮优化）**：新增 `Win11Chrome.cs`（DWM 圆角，`DWMWA_WINDOW_CORNER_PREFERENCE`=33，仅 Win11 22000+ 生效）；主窗口/启动窗口圆角跟随系统，最大化时自动恢复方角；标题栏按钮改为 Win11 风格（40×32 悬浮圆角、悬停浅灰、关闭悬停 #C42B1C 红 + 白字形，字形描边随按钮 Foreground 变色）。
