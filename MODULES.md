...
> 版本：随主程序 5.0.0。命名空间统一为 `WINHELP`；程序集名/EXE 名为「司南工具箱」（AppData 数据目录仍为 `WINHELP`）。

## UI 规范（v5.0.0，新增）

- **显示模式**：普通/专业双模式（`UiMode.cs` 全局单例 + `SettingsManager.Mode` 持久化；设置页与主窗口顶栏可切换）。
  普通模式默认：检测结果显示通俗解释（`Glossary.Hint` 追加"（术语：解释）"）、隐藏原始命令/配置名；
  专业模式显示完整技术数据。**所有检测/结果类模块必须订阅 `UiMode.Changed` 并重渲染**。
- **术语解释**：统一走 `Glossary.cs` 词典（term → 一句话中英解释），禁止在页面内散写硬编码解释。
